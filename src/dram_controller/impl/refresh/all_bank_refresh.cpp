...
class AllBankRefresh : public IRefreshManager, public Implementation {
  ...
  private:
    ...
    int m_refresh_log_interval = 4000;
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_refresh_log_interval = param<int>("refresh_log_interval").desc("Interval for logging refresh work").default_val(4000);
    };
    ...
    void tick() {
      ...
      if (m_clk == m_next_refresh_cycle) {
        m_next_refresh_cycle += m_nrefi;
        m_refresh_count++;  // refresh 카운터 증가
        ...
        if (m_refresh_count % m_refresh_log_interval == 0) {
          std::cout << "refresh work " << m_refresh_count << std::endl;
        }
      }
    };
